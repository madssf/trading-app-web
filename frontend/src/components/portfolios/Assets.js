...
export default function Assets(props) {
    ...
    let totals = [{'id':props.assets[0].id, 'value': props.assets[0].value, 'symbol': props.assets[0].symbol, 'positions': [{'value': props.assets[0].value, 'exchange': props.assets[0].exchange, 'status':  props.assets[0].status}]}]
      ...
      while (j < totals.length){
        if (totalsSymbols.includes(symbol)) {
          if (symbol === totals[j].symbol && (!(assetIds.includes(id)))) {
            ...
            var newPos = totals[j].positions.concat([{'value': props.assets[i].value, 'exchange': props.assets[i].exchange, 'status':  props.assets[i].status}])
          ...
          assetIds.push(id)
          totals.push({'id':id, 'value': value, 'symbol': symbol, 'positions': [{'value': props.assets[i].value, 'exchange': props.assets[i].exchange, 'status':  props.assets[i].status}]})
        }
        }
